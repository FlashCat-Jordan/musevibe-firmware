...
def process_emoji_collection(emoji_collection_dir, assets_dir):
    ...
    # Check if this is musevibe-gif collection
    is_musevibe_gif = 'musevibe-emoji-gif-component' in emoji_collection_dir or emoji_collection_dir.endswith('musevibe-gif')
    
    # MuseVibe GIF emoji aliases mapping
    musevibe_gif_aliases = {
        ...
        for file in files:
            if file.lower().endswith(('.png', '.gif')):
                ...
                if copy_file(src_file, dst_file):
                    ...
                    # Add aliases for musevibe-gif emojis
                    if is_musevibe_gif and filename_without_ext in musevibe_gif_aliases:
                        for alias in musevibe_gif_aliases[filename_without_ext]:
                            emoji_list.append({
                                "name": alias,
                                "file": file
                            })
...
def get_text_font_path(builtin_text_font, musevibe_fonts_path):
    """
    ...
    font_path = os.path.join(musevibe_fonts_path, 'cbin', font_name)
...
def get_emoji_collection_path(default_emoji_collection, musevibe_fonts_path, project_root=None):
    """
    Get the emoji collection path if needed
    Returns the emoji directory path or None if no emoji collection is needed
    
    Supports:
    - PNG emoji collections from musevibe-fonts (e.g., emojis_32, twemoji_64)
    - GIF emoji collections from musevibe-fonts (e.g., noto-emoji_128, noto-emoji_64)
    - MuseVibe GIF emoji collection (musevibe-gif)
    """
    if not default_emoji_collection:
        return None
    
    # Special handling for musevibe-gif collection
    if default_emoji_collection == 'musevibe-gif':
        if project_root:
            musevibe_gif_path = os.path.join(project_root, 'managed_components', 
                                        'txp666__musevibe-emoji-gif-component', 'gifs')
            if os.path.exists(musevibe_gif_path):
                return musevibe_gif_path
            else:
                print(f"Warning: MuseVibe GIF emoji collection directory not found: {musevibe_gif_path}")
                return None
        else:
            print("Warning: project_root not provided, cannot locate musevibe-gif collection")
            return None
    
    # Try PNG emoji collections first (e.g., emojis_32, twemoji_64)
    emoji_path = os.path.join(musevibe_fonts_path, 'png', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
    
    # Try GIF emoji collections (e.g., noto-emoji_128, noto-emoji_64, noto-emoji_32)
    emoji_path = os.path.join(musevibe_fonts_path, 'gif', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
...
def main():
    ...
    parser.add_argument('--musevibe_fonts_path', help='Path to musevibe-fonts component directory')
    parser.add_argument('--extra_files', help='Path to extra files directory to be included in assets')
    
    args = parser.parse_args()
    
    # Set default paths if not provided
    if not args.esp_sr_model_path or not args.musevibe_fonts_path:
        ...
        if not args.musevibe_fonts_path:
            args.musevibe_fonts_path = os.path.join(project_root, "components", "musevibe-fonts")
    ...
    text_font_path = get_text_font_path(args.builtin_text_font, args.musevibe_fonts_path)
    
    # Get emoji collection path if needed
    # Calculate project root from script location for musevibe-gif support
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    emoji_collection_path = get_emoji_collection_path(args.emoji_collection, args.musevibe_fonts_path, project_root)
