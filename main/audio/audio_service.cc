...
AudioService::~AudioService() {
    ...
    if (ambient_opus_encoder_ != nullptr) {
        esp_opus_enc_close(ambient_opus_encoder_);
    }
...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    /* 初始化环境聆听 Opus 编码器（120ms 帧，更低码率） */
    esp_opus_enc_config_t ambient_enc_cfg = AS_AMBIENT_OPUS_ENC_CONFIG();
    ret = esp_opus_enc_open(&ambient_enc_cfg, sizeof(esp_opus_enc_config_t), &ambient_opus_encoder_);
    if (ambient_opus_encoder_ == nullptr) {
        ESP_LOGE(TAG, "Failed to create ambient audio encoder, error code: %d", ret);
    } else {
        esp_opus_enc_get_frame_size(ambient_opus_encoder_, &ambient_frame_size_, &ambient_outbuf_size_);
        ambient_frame_size_ = ambient_frame_size_ / sizeof(int16_t);
    }
    ...
    audio_processor_->OnOutput([this](std::vector<int16_t>&& data) {
        /* 环境聆听：如果环境模式运行中，复制一份数据到环境编码队列 */
        if (xEventGroupGetBits(event_group_) & AS_EVENT_AMBIENT_RUNNING) {
            auto data_copy = data;
            PushTaskToEncodeQueue(kAudioTaskTypeEncodeToAmbientQueue, std::move(data_copy));
        }
    ...
    while (true) {
        ...
        if (bits & (AS_EVENT_WAKE_WORD_RUNNING | AS_EVENT_AUDIO_PROCESSOR_RUNNING)) {
            ...
            if (bits & AS_EVENT_WAKE_WORD_RUNNING) {
                samples = std::max(samples, (int)wake_word_->GetFeedSize());
            }
            if (bits & AS_EVENT_AUDIO_PROCESSOR_RUNNING) {
                samples = std::max(samples, (int)audio_processor_->GetFeedSize());
            }
            std::vector<int16_t> data;
            ...
            if (opus_encoder_ != nullptr && task->pcm.size() == encoder_frame_size_) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    if (task->type == kAudioTaskTypeEncodeToSendQueue) {
                        ...
                        /* 环境音频使用独立的 120ms 帧 Opus 编码 */
                        if (ambient_opus_encoder_ != nullptr) {
                            /* 累积 PCM 直到够 120ms 帧 (1920 samples @ 16kHz) */
                            static std::vector<int16_t> ambient_pcm_buffer;
                            ambient_pcm_buffer.insert(ambient_pcm_buffer.end(), task->pcm.begin(), task->pcm.end());

                            const int ambient_frame_size = 16000 * AMBIENT_OPUS_FRAME_DURATION_MS / 1000;  // 1920
                            while (ambient_pcm_buffer.size() >= (size_t)ambient_frame_size) {
                                auto ambient_packet = std::make_unique<AudioStreamPacket>();
                                ambient_packet->frame_duration = AMBIENT_OPUS_FRAME_DURATION_MS;
                                ambient_packet->sample_rate = 16000;
                                ambient_packet->timestamp = task->timestamp;

                                std::vector<uint8_t> buf(ambient_outbuf_size_);
                                esp_audio_enc_in_frame_t ambient_in = {
                                    .buffer = (uint8_t *)(ambient_pcm_buffer.data()),
                                    .len = (uint32_t)(ambient_frame_size * sizeof(int16_t)),
                                };
                                esp_audio_enc_out_frame_t ambient_out = {
                                    .buffer = buf.data(),
                                    .len = (uint32_t)ambient_outbuf_size_,
                                    .encoded_bytes = 0,
                                };
                                auto ambient_ret = esp_opus_enc_process(ambient_opus_encoder_, &ambient_in, &ambient_out);

                                ambient_pcm_buffer.erase(ambient_pcm_buffer.begin(), ambient_pcm_buffer.begin() + ambient_frame_size);

                                if (ambient_ret == ESP_AUDIO_ERR_OK && ambient_out.encoded_bytes > 0) {
                                    ambient_packet->payload.assign(buf.data(), buf.data() + ambient_out.encoded_bytes);
                                    if (audio_ambient_queue_.size() < MAX_AMBIENT_PACKETS_IN_QUEUE) {
                                        std::lock_guard<std::mutex> lock2(audio_queue_mutex_);
                                        audio_ambient_queue_.push_back(std::move(ambient_packet));
                                    }
                                }
                            }
                        }
...
void AudioService::EnableAmbientListening(bool enable) {
    ESP_LOGI(TAG, "%s ambient listening", enable ? "Enabling" : "Disabling");
    if (enable) {
        if (!audio_processor_initialized_) {
            audio_processor_->Initialize(codec_, OPUS_FRAME_DURATION_MS, models_list_);
            audio_processor_initialized_ = true;
        }
        xEventGroupSetBits(event_group_, AS_EVENT_AMBIENT_RUNNING);
        if (!codec_->input_enabled()) {
            codec_->EnableInput(true);
        }
        last_input_time_ = std::chrono::steady_clock::now();
    } else {
        xEventGroupClearBits(event_group_, AS_EVENT_AMBIENT_RUNNING);
        std::lock_guard<std::mutex> lock(audio_queue_mutex_);
        audio_ambient_queue_.clear();
    }
}

std::unique_ptr<AudioStreamPacket> AudioService::PopPacketFromAmbientQueue() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    if (audio_ambient_queue_.empty()) {
        return nullptr;
    }
    auto packet = std::move(audio_ambient_queue_.front());
    audio_ambient_queue_.pop_front();
    return packet;
}
