...
#define MAX_AMBIENT_PACKETS_IN_QUEUE 100  // 环境音频队列大小
...
#define AS_EVENT_AMBIENT_RUNNING            (1 << 4)  // 新增：环境聆听标志位
...
// 环境聆听使用 120ms 帧时长，更低码率
#define AMBIENT_OPUS_FRAME_DURATION_MS 120
#define AS_AMBIENT_OPUS_ENC_CONFIG() {                                                                            \
        .sample_rate        = ESP_AUDIO_SAMPLE_RATE_16K,                                                          \
        .channel            = ESP_AUDIO_MONO,                                                                     \
        .bits_per_sample    = ESP_AUDIO_BIT16,                                                                    \
        .bitrate            = 16000,                                                                              \
        .frame_duration     = (esp_opus_enc_frame_duration_t)AS_OPUS_GET_FRAME_DRU_ENUM(AMBIENT_OPUS_FRAME_DURATION_MS),\
        .application_mode   = ESP_OPUS_ENC_APPLICATION_AUDIO,                                                     \
        .complexity         = 0,                                                                                  \
        .enable_fec         = false,                                                                              \
        .enable_dtx         = true,                                                                               \
...
public:
    ...
    void EnableAmbientListening(bool enable);

    void SetCallbacks(AudioServiceCallbacks& callbacks);

    bool PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait = false);
    std::unique_ptr<AudioStreamPacket> PopPacketFromSendQueue();
    std::unique_ptr<AudioStreamPacket> PopPacketFromAmbientQueue();
...
private:
    ...
    void* ambient_opus_encoder_ = nullptr;  // 环境聆听专用 Opus 编码器
    ...
    int ambient_frame_size_ = 0;
    int ambient_outbuf_size_ = 0;
    ...
    std::deque<std::unique_ptr<AudioStreamPacket>> audio_ambient_queue_;  // 环境音频发送队列（独立于主语音）
