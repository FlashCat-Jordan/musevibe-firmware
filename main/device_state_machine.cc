...
bool DeviceStateMachine::IsValidTransition(DeviceState from, DeviceState to) const {
    ...
    switch (from) {
        ...
        case kDeviceStateActivating:
            // Can go to upgrading, idle, ambient listening, or back to wifi configuring (on error)
            return to == kDeviceStateUpgrading ||
                   to == kDeviceStateIdle ||
                   to == kDeviceStateAmbientListening ||
        ...
        case kDeviceStateIdle:
            // Can go to connecting, listening (manual mode), speaking, activating, upgrading, ambient, or wifi configuring
        ...
        case kDeviceStateAmbientListening:
            // Can go to connecting (wake word), idle (error/fallback), or upgrading
            return to == kDeviceStateConnecting ||
                   to == kDeviceStateIdle ||
                   to == kDeviceStateUpgrading;
        ...
        case kDeviceStateListening:
            // Can go to speaking, idle, or ambient_listening (conversation ended)
