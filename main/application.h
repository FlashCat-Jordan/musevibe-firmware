...
#define MAIN_EVENT_SEND_AMBIENT_AUDIO   (1 << 13)  // 新增：环境音频发送事件
#define MAIN_EVENT_AMBIENT_CONNECTED    (1 << 14)  // 新增：环境连接成功事件
...
private:
    ...
    std::unique_ptr<Protocol> ambient_protocol_;  // 新增：环境聆听 WebSocket
    ...
    void HandleAmbientSendAudioEvent();  // 新增：处理环境音频发送
    void HandleAmbientConnectedEvent();  // 新增：环境连接建立
