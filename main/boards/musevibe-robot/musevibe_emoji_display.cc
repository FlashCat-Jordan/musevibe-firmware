...
#define TAG "MuseVibeEmojiDisplay"
MuseVibeEmojiDisplay::MuseVibeEmojiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                                   int width, int height, int offset_x, int offset_y, bool mirror_x,
                                   bool mirror_y, bool swap_xy)
    : SpiLcdDisplay(panel_io, panel, width, height, offset_x, offset_y, mirror_x, mirror_y,
                    swap_xy) {}

void MuseVibeEmojiDisplay::SetupUI() {
    ...
    // 隐藏低电量弹窗 —— MuseVibe 用电池图标变色代替大弹窗，避免遮挡内容
...
void MuseVibeEmojiDisplay::SetupPreviewImage() {
    DisplayLockGuard lock(this);
...
void MuseVibeEmojiDisplay::InitializeMuseVibeEmojis() {
    ESP_LOGI(TAG, "MuseVibe表情初始化将由Assets系统处理");
    // 表情初始化已移至assets系统,通过DEFAULT_EMOJI_COLLECTION=musevibe-gif配置
    // assets.cc会从assets分区加载GIF表情并设置到theme
    // Note: Default emotion is now set in SetupUI() after LVGL objects are created
}

LV_FONT_DECLARE(MUSEVIBE_ICON_FONT);
void MuseVibeEmojiDisplay::SetStatus(const char* status) {
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        lv_obj_set_style_text_font(status_label_, &MUSEVIBE_ICON_FONT, 0);
...
void MuseVibeEmojiDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (preview_image_ == nullptr) {
...
void MuseVibeEmojiDisplay::UpdateStatusBar(bool update_all) {
    // 先调用父类更新（电池图标、网络图标、时间等）
    SpiLcdDisplay::UpdateStatusBar(update_all);

    // MuseVibe 定制：低电量时电池图标变红，而不是弹出红色弹窗
    auto& board = Board::GetInstance();
    int battery_level;
    bool charging, discharging;
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        bool is_low = !charging && battery_level <= 19;
        auto* lvgl_theme = static_cast<LvglTheme*>(current_theme_);
        DisplayLockGuard lock(this);
        if (is_low && !battery_label_was_red_) {
            lv_obj_set_style_text_color(battery_label_, lv_color_hex(0xFF0000), 0);
            battery_label_was_red_ = true;
        } else if (!is_low && battery_label_was_red_) {
            lv_obj_set_style_text_color(battery_label_, lvgl_theme->text_color(), 0);
            battery_label_was_red_ = false;
        }
    }
}