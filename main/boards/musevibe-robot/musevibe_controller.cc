...
#include "musevibe_movements.h"
#include "power_manager.h"
#include "sdkconfig.h"
#include "settings.h"
#include <wifi_manager.h>

#define TAG "MuseVibeController"

class MuseVibeController {
private:
    MuseVibe musevibe_;
    ...
    static void ActionTask(void* arg) {
        MuseVibeController* controller = static_cast<MuseVibeController*>(arg);
        MuseVibeActionParams params;
        controller->musevibe_.AttachServos();
            ...
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                if (params.action_type == ACTION_SERVO_SEQUENCE) {
                            ...
                            for (int i = 0; i < array_size; i++) {
                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        controller->musevibe_.Execute2(amplitude, center_angle, period, phase_diff, steps);
                                        ...
                                        controller->musevibe_.MoveServos(speed, servo_target);
                    ...
                    switch (params.action_type) {
                        case ACTION_WALK:
                            controller->musevibe_.Walk(params.steps, params.speed, params.direction,
                                                   params.amount);
                            break;
                        case ACTION_TURN:
                            controller->musevibe_.Turn(params.steps, params.speed, params.direction,
                                                   params.amount);
                            break;
                        case ACTION_JUMP:
                            controller->musevibe_.Jump(params.steps, params.speed);
                            break;
                        case ACTION_SWING:
                            controller->musevibe_.Swing(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_MOONWALK:
                            controller->musevibe_.Moonwalker(params.steps, params.speed, params.amount,
                                                         params.direction);
                            break;
                        case ACTION_BEND:
                            controller->musevibe_.Bend(params.steps, params.speed, params.direction);
                            break;
                        case ACTION_SHAKE_LEG:
                            controller->musevibe_.ShakeLeg(params.steps, params.speed, params.direction);
                            break;
                        case ACTION_SIT:
                            controller->musevibe_.Sit();
                            break;
                        case ACTION_RADIO_CALISTHENICS:
                            if (controller->has_hands_) {
                                controller->musevibe_.RadioCalisthenics();
                            }
                            break;
                        case ACTION_MAGIC_CIRCLE:
                            if (controller->has_hands_) {
                                controller->musevibe_.MagicCircle();
                            }
                            break;
                        case ACTION_SHOWCASE:
                            controller->musevibe_.Showcase();
                            break;
                        case ACTION_UPDOWN:
                            controller->musevibe_.UpDown(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_TIPTOE_SWING:
                            controller->musevibe_.TiptoeSwing(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_JITTER:
                            controller->musevibe_.Jitter(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_ASCENDING_TURN:
                            controller->musevibe_.AscendingTurn(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_CRUSAITO:
                            controller->musevibe_.Crusaito(params.steps, params.speed, params.amount,
                                                       params.direction);
                            break;
                        case ACTION_FLAPPING:
                            controller->musevibe_.Flapping(params.steps, params.speed, params.amount,
                                                       params.direction);
                            break;
                        case ACTION_WHIRLWIND_LEG:
                            controller->musevibe_.WhirlwindLeg(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_HANDS_UP:
                            if (controller->has_hands_) {
                                controller->musevibe_.HandsUp(params.speed, params.direction);
                            }
                            break;
                        case ACTION_HANDS_DOWN:
                            if (controller->has_hands_) {
                                controller->musevibe_.HandsDown(params.speed, params.direction);
                            }
                            break;
                        case ACTION_HAND_WAVE:
                            if (controller->has_hands_) {
                                controller->musevibe_.HandWave( params.direction);
                            }
                            break;
                        case ACTION_WINDMILL:
                            if (controller->has_hands_) {
                                controller->musevibe_.Windmill(params.steps, params.speed, params.amount);
                            }
                            break;
                        case ACTION_TAKEOFF:
                            if (controller->has_hands_) {
                                controller->musevibe_.Takeoff(params.steps, params.speed, params.amount);
                            }
                            break;
                        case ACTION_FITNESS:
                            if (controller->has_hands_) {
                                controller->musevibe_.Fitness(params.steps, params.speed, params.amount);
                            }
                            break;
                        case ACTION_GREETING:
                            if (controller->has_hands_) {
                                controller->musevibe_.Greeting(params.direction, params.steps);
                            }
                            break;
                        case ACTION_SHY:
                            if (controller->has_hands_) {
                                controller->musevibe_.Shy(params.direction, params.steps);
                            }
                            break;
                        case ACTION_HOME:
                            controller->musevibe_.Home(true);
                            break;
                    }
                    if(params.action_type != ACTION_SIT){
                        if (params.action_type != ACTION_HOME && params.action_type != ACTION_SERVO_SEQUENCE) {
                            controller->musevibe_.Home(params.action_type != ACTION_HANDS_UP);
                        }
                    }
                }
    ...
    void StartActionTaskIfNeeded() {
        if (action_task_handle_ == nullptr) {
            xTaskCreate(ActionTask, "musevibe_action", 1024 * 3, this, configMAX_PRIORITIES - 1,
                        &action_task_handle_);
        }
    }
    ...
    void QueueAction(int action_type, int steps, int speed, int direction, int amount) {
        ...
        MuseVibeActionParams params = {action_type, steps, speed, direction, amount, ""};
    ...
    void QueueServoSequence(const char* servo_sequence_json) {
        ...
        MuseVibeActionParams params = {ACTION_SERVO_SEQUENCE, 0, 0, 0, 0, ""};
        // 复制JSON字符串到结构体中（限制长度）
    ...
    void LoadTrimsFromNVS() {
        Settings settings("musevibe_trims", false);
        ...
        musevibe_.SetTrims(left_leg, right_leg, left_foot, right_foot, left_hand, right_hand);
    }

public:
    MuseVibeController(const HardwareConfig& hw_config) {
        musevibe_.Init(
        ...
        ESP_LOGI(TAG, "MuseVibe机器人初始化%s手部舵机", has_hands_ ? "带" : "不带");
        ...
        action_queue_ = xQueueCreate(10, sizeof(MuseVibeActionParams));
    ...
    void RegisterMcpTools() {
        ...
        // 统一动作工具（除了舵机序列外的所有动作）
        mcp_server.AddTool("self.musevibe.action",
        ...
        mcp_server.AddTool(
            ...
            "重要说明：左右腿脚震荡的时候，有一只脚必须在90度，否则会损坏机器人，如果发送多个序列（序列数>1），完成所有序列后需要复位时，AI应该最后单独调用self.musevibe.home工具进行复位，不要在序列中设置复位参数。"
        ...
        mcp_server.AddTool("self.musevibe.stop", "立即停止所有动作并复位", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               if (action_task_handle_ != nullptr) {
        ...
        mcp_server.AddTool(
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // 获取当前所有微调值
                Settings settings("musevibe_trims", true);
                int left_leg = settings.GetInt("left_leg", 0);
                int right_leg = settings.GetInt("right_leg", 0);
                int left_foot = settings.GetInt("left_foot", 0);
                int right_foot = settings.GetInt("right_foot", 0);
        ...
        mcp_server.AddTool("self.musevibe.get_trims", "获取当前的舵机微调设置", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               Settings settings("musevibe_trims", false);
        ...
        mcp_server.AddTool("self.musevibe.get_status", "获取机器人状态，返回 moving 或 idle",
        ...
        mcp_server.AddTool("self.musevibe.get_ip", "获取机器人WiFi IP地址", PropertyList(),
                           [](const PropertyList& properties) -> ReturnValue {
...
static MuseVibeController* g_musevibe_controller = nullptr;

void InitializeMuseVibeController(const HardwareConfig& hw_config) {
    if (g_musevibe_controller == nullptr) {
        g_musevibe_controller = new MuseVibeController(hw_config);
        ESP_LOGI(TAG, "MuseVibe控制器已初始化并注册MCP工具");
    }
}
