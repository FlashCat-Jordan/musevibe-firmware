...
static const char* TAG = "MuseVibeMovements";

#define HAND_HOME_POSITION 45

MuseVibe::MuseVibe() {
    is_musevibe_resting_ = false;
    has_hands_ = false;
    // 初始化所有舵机管脚为-1（未连接）
...
void MuseVibe::Init(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand,
                int right_hand) {
...
//-- OSCILLATORS TRIMS ------------------------------------------//
///////////////////////////////////////////////////////////////////
void MuseVibe::SetTrims(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand,
                    int right_hand) {
...
///////////////////////////////////////////////////////////////////
void MuseVibe::MoveServos(int time, int servo_target[]) {
...
void MuseVibe::MoveSingle(int position, int servo_number) {
    if (position > 180)
        position = 90;
    if (position < 0)
        position = 90;
...
void MuseVibe::OscillateServos(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
...
void MuseVibe::Execute(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
...
//--    steps: 步数/周期数（可为小数）
//---------------------------------------------------------
void MuseVibe::Execute2(int amplitude[SERVO_COUNT], int center_angle[SERVO_COUNT], int period,
                    double phase_diff[SERVO_COUNT], float steps = 1.0) {
    if (GetRestState() == true) {
        SetRestState(false);
    }
...
///////////////////////////////////////////////////////////////////
//-- HOME = MuseVibe at rest position -------------------------------//
///////////////////////////////////////////////////////////////////
void MuseVibe::Home(bool hands_down) {
    if (is_musevibe_resting_ == false) {  // Go to rest position only if necessary
        // 为所有舵机准备初始位置值
        ...
        is_musevibe_resting_ = true;
    }

    vTaskDelay(pdMS_TO_TICKS(200));
}

bool MuseVibe::GetRestState() {
    return is_musevibe_resting_;
}

void MuseVibe::SetRestState(bool state) {
    is_musevibe_resting_ = state;
}

///////////////////////////////////////////////////////////////////
//-- PREDETERMINED MOTION SEQUENCES -----------------------------//
///////////////////////////////////////////////////////////////////
//-- MuseVibe movement: Jump
//--  Parameters:
//--    steps: Number of steps
//--    T: Period
//---------------------------------------------------------
void MuseVibe::Jump(float steps, int period) {
...
//---------------------------------------------------------
//-- MuseVibe gait: Walking  (forward or backward)
//--  Parameters:
//--    * steps:  Number of steps
//--    * T : Period
...
//---------------------------------------------------------
void MuseVibe::Walk(float steps, int period, int dir, int amount) {
...
//---------------------------------------------------------
//-- MuseVibe gait: Turning (left or right)
//--  Parameters:
//--   * Steps: Number of steps
//--   * T: Period
...
//--   * amount: 手部摆动幅度, 0表示不摆动
//---------------------------------------------------------
void MuseVibe::Turn(float steps, int period, int dir, int amount) {
    //-- Same coordination than for walking (see MuseVibe::walk)
...
//---------------------------------------------------------
//-- MuseVibe gait: Lateral bend
//--  Parameters:
//--    steps: Number of bends
//--    T: Period of one bend
//--    dir: RIGHT=Right bend LEFT=Left bend
//---------------------------------------------------------
void MuseVibe::Bend(int steps, int period, int dir) {
    ...
    if (dir == -1) {
        bend1[2] = 180 - 35;
        bend1[3] = 180 - 60;  // Not 65. MuseVibe is unbalanced
        bend2[2] = 180 - 105;
        bend2[3] = 180 - 60;
    }
...
//---------------------------------------------------------
//-- MuseVibe gait: Shake a leg
//--  Parameters:
//--    steps: Number of shakes
//--    T: Period of one shake
//--    dir: RIGHT=Right leg LEFT=Left leg
//---------------------------------------------------------
void MuseVibe::ShakeLeg(int steps, int period, int dir) {
...
//---------------------------------------------------------
void MuseVibe::UpDown(float steps, int period, int height) {
...
//---------------------------------------------------------
//-- MuseVibe movement: swinging side to side
//--  Parameters:
//--     steps: Number of steps
//--     T : Period
//--     h : Amount of swing (from 0 to 50 aprox)
//---------------------------------------------------------
void MuseVibe::Swing(float steps, int period, int height) {
...
//---------------------------------------------------------
//-- MuseVibe movement: swinging side to side without touching the floor with the heel
//--  Parameters:
//--     steps: Number of steps
//--     T : Period
//--     h : Amount of swing (from 0 to 50 aprox)
//---------------------------------------------------------
void MuseVibe::TiptoeSwing(float steps, int period, int height) {
...
//---------------------------------------------------------
//-- MuseVibe gait: Jitter
//--  Parameters:
//--    steps: Number of jitters
//--    T: Period of one jitter
//--    h: height (Values between 5 - 25)
//---------------------------------------------------------
void MuseVibe::Jitter(float steps, int period, int height) {
    //-- Both feet are 180 degrees out of phase
    //-- Feet amplitude and offset are the same
    //-- Initial phase for the right foot is -90, so that it starts
    //--   in one extreme position (not in the middle)
...
//---------------------------------------------------------
//-- MuseVibe gait: Ascending & turn (Jitter while up&down)
//--  Parameters:
//--    steps: Number of bends
//--    T: Period of one bend
//--    h: height (Values between 5 - 15)
//---------------------------------------------------------
void MuseVibe::AscendingTurn(float steps, int period, int height) {
...
//---------------------------------------------------------
//-- MuseVibe gait: Moonwalker. MuseVibe moves like Michael Jackson
//--  Parameters:
//--    Steps: Number of steps
//--    T: Period
...
//--    dir: Direction: LEFT / RIGHT
//---------------------------------------------------------
void MuseVibe::Moonwalker(float steps, int period, int height, int dir) {
    //-- This motion is similar to that of the caterpillar robots: A travelling
    //-- wave moving from one side to another
    //-- The two MuseVibe's feet are equivalent to a minimal configuration. It is known
    //-- that 2 servos can move like a worm if they are 120 degrees out of phase
    //-- In the example of MuseVibe, the two feet are mirrored so that we have:
...
//----------------------------------------------------------
//-- MuseVibe gait: Crusaito. A mixture between moonwalker and walk
//--   Parameters:
//--     steps: Number of steps
//--     T: Period
...
//--     dir:  Direction: LEFT / RIGHT
//-----------------------------------------------------------
void MuseVibe::Crusaito(float steps, int period, int height, int dir) {
...
//---------------------------------------------------------
void MuseVibe::Flapping(float steps, int period, int height, int dir) {
...
//---------------------------------------------------------
//-- MuseVibe gait: WhirlwindLeg (旋风腿)
//--   Parameters:
//--     steps: Number of steps
//--     period: Period (建议100-800毫秒)
//--     amplitude: amplitude (Values between 20 - 40)
//---------------------------------------------------------
void MuseVibe::WhirlwindLeg(float steps, int period, int amplitude) {
...
//---------------------------------------------------------
void MuseVibe::HandsUp(int period, int dir) {
    if (!has_hands_) {
        return;
    }
...
//--    dir: 方向 1=左手, -1=右手, 0=双手
//---------------------------------------------------------
void MuseVibe::HandsDown(int period, int dir) {
    if (!has_hands_) {
        return;
    }
...
//---------------------------------------------------------
void MuseVibe::HandWave(int dir) {
    if (!has_hands_) {
        return;
    }
    if (dir == LEFT) {
...
//--    amplitude: 振荡幅度（度）
//---------------------------------------------------------
void MuseVibe::Windmill(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
//--    amplitude: 振荡幅度（度）
//---------------------------------------------------------
void MuseVibe::Takeoff(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
//--    amplitude: 振荡幅度（度）
//---------------------------------------------------------
void MuseVibe::Fitness(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
//--    steps: 动作次数
//---------------------------------------------------------
void MuseVibe::Greeting(int dir, float steps) {
    if (!has_hands_) {
        return;
    }
    if (dir == LEFT) {
...
//--    steps: 动作次数
//---------------------------------------------------------
void MuseVibe::Shy(int dir, float steps) {
    if (!has_hands_) {
        return;
    }
...
//-- 手部动作: 广播体操
//---------------------------------------------------------
void MuseVibe::RadioCalisthenics() {
    if (!has_hands_) {
        return;
    }
...
void MuseVibe::EnableServoLimit(int diff_limit) {
    for (int i = 0; i < SERVO_COUNT; i++) {
        if (servo_pins_[i] != -1) {
...
void MuseVibe::DisableServoLimit() {
    for (int i = 0; i < SERVO_COUNT; i++) {
        if (servo_pins_[i] != -1) {
