...
LcdDisplay::~LcdDisplay() {
    ...
    if (bmusevibem_bar_ != nullptr) {
        lv_obj_del(bmusevibem_bar_);
    }
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_pad_bmusevibem(top_bar_, lvgl_theme->spacing(2), 0);
    ...
    lv_obj_set_style_pad_bmusevibem(status_bar_, lvgl_theme->spacing(2), 0);
    ...
    lv_obj_align(low_battery_popup_, LV_ALIGN_BMUSEVIBEM_MID, 0, -lvgl_theme->spacing(4));
...
void LcdDisplay::SetupUI() {
    ...
    /* Bmusevibem layer: emoji_box_ - centered display */
    ...
    /* Bmusevibem bar - auto height, grows upward with wrapped text */
    bmusevibem_bar_ = lv_obj_create(screen);
    lv_obj_set_width(bmusevibem_bar_, LV_HOR_RES);
    lv_obj_set_height(bmusevibem_bar_, LV_SIZE_CONTENT);
    lv_obj_set_style_radius(bmusevibem_bar_, 0, 0);
    lv_obj_set_style_bg_color(bmusevibem_bar_, lvgl_theme->background_color(), 0);
    lv_obj_set_style_bg_opa(bmusevibem_bar_, LV_OPA_50, 0);
    lv_obj_set_style_text_color(bmusevibem_bar_, lvgl_theme->text_color(), 0);
    lv_obj_set_style_pad_all(bmusevibem_bar_, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_border_width(bmusevibem_bar_, 0, 0);
    lv_obj_set_scrollbar_mode(bmusevibem_bar_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_align(bmusevibem_bar_, LV_ALIGN_BMUSEVIBEM_MID, 0, 0);

    /* chat_message_label_ placed in bmusevibem_bar_, multiline wrapped display */
    chat_message_label_ = lv_label_create(bmusevibem_bar_);
    ...
    lv_obj_add_flag(bmusevibem_bar_, LV_OBJ_FLAG_HIDDEN);  // Hide until there is content
#else
    /* Top layer: Bmusevibem bar - fixed height at bmusevibem */
    bmusevibem_bar_ = lv_obj_create(screen);
    lv_obj_set_size(bmusevibem_bar_, LV_HOR_RES, text_font->line_height + lvgl_theme->spacing(8));
    lv_obj_set_style_radius(bmusevibem_bar_, 0, 0);
    lv_obj_set_style_bg_color(bmusevibem_bar_, lvgl_theme->background_color(), 0);
    lv_obj_set_style_text_color(bmusevibem_bar_, lvgl_theme->text_color(), 0);
    lv_obj_set_style_pad_all(bmusevibem_bar_, 0, 0);
    lv_obj_set_style_pad_left(bmusevibem_bar_, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_pad_right(bmusevibem_bar_, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_border_width(bmusevibem_bar_, 0, 0);
    lv_obj_set_scrollbar_mode(bmusevibem_bar_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_align(bmusevibem_bar_, LV_ALIGN_BMUSEVIBEM_MID, 0, 0);

    /* chat_message_label_ placed in bmusevibem_bar_, single-line horizontal scroll */
    chat_message_label_ = lv_label_create(bmusevibem_bar_);
    lv_label_set_text(chat_message_label_, "");
    lv_obj_set_width(chat_message_label_, LV_HOR_RES - lvgl_theme->spacing(8));
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Show bmusevibem_bar_ only when there is content (and subtitle is not globally hidden)
    if (bmusevibem_bar_ != nullptr) {
        if (content == nullptr || content[0] == '\0') {
            lv_obj_add_flag(bmusevibem_bar_, LV_OBJ_FLAG_HIDDEN);
        } else if (!hide_subtitle_) {
            lv_obj_remove_flag(bmusevibem_bar_, LV_OBJ_FLAG_HIDDEN);
        }
    }
#if CONFIG_USE_MULTILINE_CHAT_MESSAGE
    // Re-align bmusevibem_bar_ after text change so it stays anchored to the bmusevibem
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Update bmusevibem bar background color with 50% opacity
    if (bmusevibem_bar_ != nullptr) {
        lv_obj_set_style_bg_opa(bmusevibem_bar_, LV_OPA_50, 0);
        lv_obj_set_style_bg_color(bmusevibem_bar_, lvgl_theme->background_color(), 0);
    }
